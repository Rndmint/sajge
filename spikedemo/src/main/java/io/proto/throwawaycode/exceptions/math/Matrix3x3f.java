...
public class Matrix3x3f {
    ...
    public Matrix3x3f(float[][] array) {
        ...
        this.data = new float[3][3];
        for (int i = 0; i < 3; i++) {
            System.arraycopy(array[i], 0, this.data[i], 0, 3);
        }
    }
    ...
    public Matrix3x3f inverse() {
        ...
        if (det == 0f) throw new SingularMatrixException("Matrix3x3f is not invertible: " + this);
        return computeInverse(det);
    }

    private Matrix3x3f computeInverse(float det) {
        float[][] m = data;
